...
test.describe('API Journey: Web Frontend HTTP Endpoints', () => {
  ...
  test('should make the found-photo upload keyboard accessible', async ({ page }) => {
  ...
  test('should keep home navigation actions visible and keyboard accessible on phones', async ({ page, context }) => {
    ...
    for (const id of ['nav-home', 'nav-directory', 'nav-matches', 'nav-analytics', 'nav-evacuation', ...actionIds]) {
